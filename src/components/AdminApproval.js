...
class AdminApproval extends Component {
  ...
  recipeStatus(status, recipeId) {
    axios
      .post(
        `${this.url}/recipe/approval`,
        { approved: status, id: recipeId },
        {
          headers: {
            "x-access-token": "Bearer " + localStorage.getItem("token"),
          },
        ...
        <Table sx={{ minWidth: 700 }} aria-label="customized table">
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell align="center">ingredientLines</StyledTableCell>
          ...
          <TableBody>
            {this.state.recipes.map((row) => (
              <StyledTableRow key={row.name}>
                ...
                <StyledTableCell align="center">
                  <Button
                    onClick={() => this.recipeStatus("approved", row["_id"])}
                  >
                    Approve
                  </Button>
                </StyledTableCell>
                <StyledTableCell align="center">
                  <Button
                    onClick={() => this.recipeStatus("rejected", row["_id"])}
                  >
                    Reject
                  </Button>
                </StyledTableCell>
